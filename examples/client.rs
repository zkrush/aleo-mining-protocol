use aleo_mining_protocol::{PoolMessage, AuthRequest};
...
async fn main() -> Result<()>{
    ...
    info!("[aleo-mining-protocol] receive auth response: {:?}", new_task);
...
pub async fn send_frame<N: Network>(outgoing: &mut SplitSink<WebSocketStream<MaybeTlsStream<TcpStream>>, Message>, message: PoolMessage<N>) -> Result<()>{
    let ws_message = tokio_tungstenite::tungstenite::Message::text(serde_json::to_string(&message)?);
    outgoing.send(ws_message).await?;
    Ok(())
}

pub async fn read_frame<N: Network>(incoming: &mut SplitStream<WebSocketStream<MaybeTlsStream<TcpStream>>>) -> Result<PoolMessage<N>> {
